...
int main1() {
    ...
    cout << x;
    fprintf(pipe, "plot [0 : 3] [3 : 70] %lf*x**3 + %lf*x**2 + %lf*x**1 + %lf*x**0 , '-' using 1:2 with points\n",
            x.matrix[3][0], x.matrix[2][0], x.matrix[1][0], x.matrix[0][0]);
    fprintf(pipe, "plot '-' w p ls 1, '-' w p ls 2, '-' w p ls 3, '-' w p ls 4, '-' w p ls 5, '-' w p ls 6,"
                  " '-' w p ls 7, '-' w p ls 8, '-' w p ls 9, '-' w p ls 10\n");
    for (int i = 0; i < m; i++) {
        fprintf(pipe, "%f\t%f\n", t[i], b.matrix[i][0]);
    }
#ifdef WIN32
